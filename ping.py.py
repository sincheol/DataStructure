...
def recv_echo_resp(sock, id, seq, timeout):
    ...
    try:
        while True:
            ...
            echo_resp = ICMPHdr.packet_analysis(ip.data)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='ping by torry')
    ...
    try:
        ...
        print('Ping {0} ({1}) {2}({3}) bytes of data'.format(dst_addr, host, length, length + ICMPHdr.ICMP_DEFAULT_SIZE + 20))

    except socket.gaierror:
        print('Cannot resolve host ip address from {0}'.format(host))
        sys.exit(-1)
    ...
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_RAW, socket.IPPROTO_ICMP) as sock:
            ...
            for echo_req in gen_echo_req(count=count, data_len=length, id=echo_id):
                sock.send(echo_req.encapsulation())
                sent_time = time.time()
                sent_packet_count += 1

                received_time, src_addr, data_length, ttl, seq = recv_echo_resp(sock=sock, id=echo_req.id, seq=echo_req.seq, timeout=timeout)
                ...
                if received_time:
                    ...
                    print('{data_length} bytes from {src_addr} : icmp seq = {seq} ttl={ttl} time = {rtt:.3f} ms'.format(
                        data_length=data_length,
                        src_addr=src_addr,
                        seq=seq, ttl=ttl, rtt=rtt*1000))
    ...
    print_ping_statistics(dst_addr=dst_addr, sent_packets=sent_packet_count, recv_packets=recv_packet_count, rtts=rtt_list)
