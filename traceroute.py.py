...
class IPHdr:
    ...
    @classmethod
    def packet_analysis(cls, raw_data):
        ip_hdr = IPHdr()
...
class ICMPHdr:
    ...
    def __init__(self, type=8, code=0, checksum=0, id=0, seq=2018037057, data=''): #constructor
        self.type = type
        self.code = code
...
class UDPHdr:
    ...
    def packet_analysis(cls, raw_data):
        ...
        udp_hdr.src_port, udp_hdr.dst_port, udp_hdr.packet_length, udp_hdr.checksum = struct.unpack(
            "!HHHH", raw_data
        )
...
def recv_resp(sock, echo_id, protocol, timeout, sent_packet):
    ...
    try:
        while True:
            ...
            ip = IPHdr.packet_analysis(pkt)
            icmp = ICMPHdr.packet_analysis(ip.data)
...
def check_response_udp(sent_packet, recv_packet):
    ref_ip_hdr = IPHdr.packet_analysis(recv_packet.data)
    ref_udp_hdr = UDPHdr.packet_analysis(ref_ip_hdr.data)

    return (
        ref_udp_hdr.src_port == sent_packet.src_port
        and ref_udp_hdr.dst_port == sent_packet.dst_port
    )


def check_response_icmp(sent_packet, recv_packet):
    ref_ip_hdr = IPHdr.packet_analysis(recv_packet.data)
    ref_icmp_hdr = ICMPHdr.packet_analysis(ref_ip_hdr.data)

    return ref_icmp_hdr.id == sent_packet.id

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="simple traceroute by delee")
    ...
    parser.add_argument(
        ...
        nargs="?",
        metavar="ICMP_ECHO",
        help="using ICMP_ECHO_REQUEST",
        const=socket.IPPROTO_ICMP
    )
    parser.add_argument(
        "-U", type=int, nargs="?", metavar="UDP", help="using UDP", const=socket.IPPROTO_UDP
    )
    parser.add_argument(
        "-t", type=float, metavar="RECV_TIME_OUT",help="request timeout", default=1
    )
    ...
    try:
        ...
        print(
            "traceroute to {0} ({1}), {2} hops max, {3} byte packets".format(
                host, dst_addr, max_hop, packet_size
            )
        )
    ...
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_RAW, send_protocol) as send_socket:
            ...
            for hop in range(1, max_hop + 1):
                ...
                for request_packet in generate_request(
                    sock=send_socket,
                    count = 3,
                    echo_id=echo_id,
                    length=packet_size,
                    ...
                    with socket.socket(
                        ...
                        send_socket.send(request_packet.encapsulation())
                        send_time = time.time()

                        received_time, addr, finished = recv_resp(
                            sock=recv_socket,
                            echo_id = echo_id,
                            protocol=send_protocol,
                            timeout=timeout,
                        ...
                        if received_time:
                            messages[seq] = " {rtt:.3f} ms".format(
                                rtt=(received_time - send_time)*1000)
                            got_response = True
                        else:
                            messages[seq] ="*"
                ...
                if got_response and received_time:
                    try:
                        messages[tries] = "[{0},{1}]".format(
                            socket.gethostbyaddr(addr[0])[0], addr[0])
                    except socket.gaierror:
        ...
        print("No permssion for creating raw socket, try superuser...")
        sys.exit(-1)
